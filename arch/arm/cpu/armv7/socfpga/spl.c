...
u32 spl_boot_device(void)
{
	...
	//Added code to copy u-boot binary from FPGA blockRAM to SDRAM
        void* dest = (void*)(0x01000000);
	void* src  = (void*)(0xC0040000);
	printf("Copying u-boot from 0x%x to 0x%x ... ", src, dest);
	memcpy(dest, src, 255*1024);
        printf("DONE\n");
	printf("Comparing from 0x%x to 0x%x ... ", src, dest);
	if(memcmp(src,dest,255*1024))
		printf("ERROR!!\n");
	else
		printf("OK !!\n");
	//Done
...
#error "CONFIG_PRELOADER_SKIP_SDRAM must be defined"
	 ...
	 */
	if (hps_emif_diag_test(SDRAM_TEST_NORMAL, 0, sdram_size) == 0)
		hang();
