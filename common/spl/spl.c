...
void spl_parse_image_header(const struct image_header *header)
{
	...
	if (image_get_magic(header) == IH_MAGIC) {
		...
		printf("spl_image.entry_point=0x%x spl_image.load_addr=0x%x\n",spl_image.entry_point,spl_image.load_addr);
	}
	printf("spl_parse_image_header--\n");
}
...
__weak void __noreturn jump_to_image_no_args(struct spl_image_info *spl_image)
{
	...
	if (spl_image->crc_size != 0) {
		 ...
		 */
		printf("Padding the SDRAM to avoid false ECC DBE\n");
		...
		if (calculated_crc != spl_image->crc) {
			...
			printf("CRC calculate from 0x%08x "
				"with length 0x%08x\n",
				spl_image->entry_point, spl_image->size);
			printf("CRC Result : Expected 0x%08x "
				"Calculated 0x%08x\n",
	...
	for(i=0;i<50;i++)
	{
		int* x = (int*)(spl_image->entry_point)+i;
		printf("0x%x\n",*x);
	}
